...
from dataloaders.dataloader import Dataset, Val_Dataset
from models.stfts import mag_phase_stft, mag_phase_istft
from models.generator import MambAttentionSEUNet
...
def train(rank, args, cfg):
    ...
    generator = MambAttentionSEUNet(cfg).to(device)
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            ...
                            if noisy_audio.size(1) >= cfg['training_cfg']['segment_size']:
                                ...
                                # If speech length is less than segment_size, fill with zeros directly
                            ...
                            # Process each segment and concatenate results
                            processed_segments = []
                            audio_g = []
                            ...
                            #Concat all secments into full speech signals

                            processed_audio = torch.cat(processed_segments, dim=-1)

                            # Crop the end and keep original length of the noisy input
